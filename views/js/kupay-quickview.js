prestashop.on('clickQuickView', function(res) {
    const mutationObserver = new MutationObserver(entry => {
        entry.forEach(element => {
            if (element.addedNodes[0] === undefined) return;
            if (element.addedNodes[0].className !== 'modal fade quickview') return;
            ...
            node.setAttribute("id", "kupay-buy");
            node.setAttribute("onclick", "kupayPdpQuickviewCheckout()");

            const elementDiv = document.getElementsByClassName(
                "product-add-to-cart"
            )[0];
            // Gets the Kupay Button if it exists
            const kupayBtn = document.getElementById("kupay-buy");

            if (elementDiv) {
                // If the button was already added, remove it
                if (kupayBtn) {
                    kupayBtn.remove();
                }
                elementDiv.appendChild(node);
            }
            if (kupay.productStyles) {
                node.setAttribute("style", kupay.productStyles);
            }
        });
    });
